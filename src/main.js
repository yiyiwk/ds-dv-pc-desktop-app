...
const { ElectronChromeExtensions } = require('../packages/electron-chrome-extensions')
const { setupMenu } = require('./menu')
const { buildChromeContextMenu } = require('../packages/electron-chrome-context-menu')
...
class Browser {
  ...
  async init() {
    ...
    const newTabUrl = 'https://www.511ds.com/'
    ...
    const win = this.createWindow({ initialUrl: newTabUrl })
    // 文件下载会打开新窗口解决方案
    // 监听文件下载，触发文件下载的时候将 文件下载打开的窗口关闭
    // 缺点 在大文件下载的时候会出现窗口显示很久才会关闭
    win.webContents.session.on('will-download', (event, item) => {
      console.log('will-download')
      // 获取最后一个tab窗口，然后关闭
      const lastTab = win.tabs.tabList[win.tabs.tabList.length - 1]
      lastTab.destroy()
    })
  }
